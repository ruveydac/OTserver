...
export const AssetObservations: CollectionConfig = {
  ...
  access: { create: () => false, delete: () => false, read: canReadSiteDocuments, update: () => false },
  admin: { hidden: true, useAsTitle: 'source' },
  defaultSort: '-observedAt',
  disableBulkDelete: true,
  disableBulkEdit: true,
  fields: [
    { name: 'site', type: 'relationship', index: true, maxDepth: 0, relationTo: 'sites', required: true },
    { name: 'asset', type: 'relationship', index: true, maxDepth: 0, relationTo: 'assets', required: true },
    { name: 'import', type: 'relationship', index: true, maxDepth: 0, relationTo: 'asset-imports', required: true },
